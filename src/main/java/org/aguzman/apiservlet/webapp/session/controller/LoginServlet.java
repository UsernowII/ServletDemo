...
import org.aguzman.apiservlet.webapp.session.services.LoginServiceCookieImpl;
...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (USERNAME.equals(username) && PASSWORD.equals(password)) {
            //creacion de session recibe nombre y valor (Map)
            // la sesion la guarda el servidor
            HttpSession session = req.getSession();
            session.setAttribute("username", username);//devuelve una cookie con el id de la session la guarda el cliente
